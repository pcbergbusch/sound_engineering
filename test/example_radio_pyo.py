...
class RunFm:
    def __init__(self, TM, TAPS, changeFreqSelect, fmTranspo, countCloseDisto, finalFadeout, IndSwitch, num=4):
        ...
        self.mCar = Beat(time=TM, taps=TAPS, w1=[90, 80, 40, 50], w2=[30, 40, 90, 40], w3=[40, 30, 30, 20],
                         poly=1).play()
        self.mRat = Beat(time=TM, taps=TAPS, w1=[50, 80, 40, 70], w2=[30, 40, 50, 80], w3=[30, 40, 70, 50],
                         poly=1).play()
        self.mInd = Beat(time=TM, taps=TAPS, w1=[30, 30, 80, 70], w2=[30, 50, 80, 30], w3=[40, 40, 80, 30],
                         poly=1).play()

        self.carAdd = TrigChoice(self.changeFreqSelect, choice=[midiToHz(x) for x in [31, 36, 43, 48, 55, 60]],
                                 mul=[rnd(.99, 1.01) for i in range(4)])
        self.mulCarMul = TrigChoice(self.changeFreqSelect, choice=[midiToHz(x) for x in [19, 24, 36, 43, 48, 55]],
                                    mul=[rnd(.99, 1.01) for i in range(4)])
        ...
        self.car = TrigEnv(self.mCar, table=self.env, dur=self.mCar['dur'], mul=self.mulCar,
                           add=self.carAdd * self.trigFmTranspo)
        self.rat = TrigEnv(self.mRat, table=self.env, dur=self.mRat['dur'], mul=self.mulRat,
                           add=[.996, .7485, 1.502, .506])
...
class RunKick:
    def __init__(self, TM, TAPS, BASS_SEQ, changeFreqSelect, ampSwitch, bassSwitch, distoSwitch):
        ...
        self.notes = DataTable(TAPS * 2, init=[midiToHz(x + 12) for x in
                                               [24, 26, 27, 29, 27, 29, 27, 26, 24, 22, 20, 22, 19, 20, 22, 26,
                                                24, 22, 26, 24, 26, 29, 27, 26, 27, 29, 27, 24, 26, 27, 26, 22]])
        self.notes2 = DataTable(TAPS * 2, init=[midiToHz(x + 7) for x in
                                                [24, 22, 24, 22, 24, 26, 27, 29, 27, 29, 27, 26, 24, 22, 26, 27,
                                                 24, 22, 26, 24, 27, 26, 29, 27, 29, 27, 26, 24, 22, 24, 27, 26]])
        self.notes3 = DataTable(TAPS * 2, init=[midiToHz(x + 12) for x in
                                                [24, 26, 27, 26, 24, 22, 24, 20, 24, 22, 20, 22, 24, 27, 22, 26,
...
class RunStrings:
    def __init__(self, TM, TAPS, SCALES, invAmpSwitch, finalFadeout, num=5):
        ...
        self.mHarm = Beat(time=TM * 8, taps=TAPS, w1=[100, 20, 0, 30, 20, 40], w2=[10, 20, 100, 20, 40, 25],
                          w3=[0, 20, 10, 40, 10, 0]).play()
        self.env = Port(self.mHarm['amp'], .75, .75, mul=2.3).stop()
        self.harmMid = TrigXnoiseMidi(self.mHarm, dist=11, x1=1, x2=.4, scale=0,
                                      mrange=[(36, 45), (45, 60), (55, 69), (64, 75), (68, 85), (72, 88)]).stop()
        ...
        self.harmOsc = SineLoop(freq=self.harmFreq, feedback=self.harmFeedRnd,
                                mul=self.env * [.055, .05, .047, .047, .035, .027]).stop()
...
RES = 'off'
ATTR = {
    'off': {'sr': 44100, 'bufsize': 512, 'audio': 'offline', 'fm': 4, 'sines': 4, 'wind': 4, 'punch': 4, 'strings': 8,
            'bell': 4}}
